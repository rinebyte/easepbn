...
import { Plus, Trash2, RotateCcw, ExternalLink, Loader2, XCircle, Zap, Download } from 'lucide-react'
...
function statusVariant(status: Post['status']) {
  const map: Record<Post['status'], 'warning' | 'info' | 'success' | 'destructive' | 'secondary'> = {
    pending: 'warning',
    posting: 'info',
    posted: 'success',
    failed: 'destructive',
...
export function PostsPage() {
  ...
  // Keep articles query only for the "New Post" dialog article selector
                      ...
                      <TableRow key={post.id}>
                        <TableCell className="font-medium">
                          <span className="block max-w-[200px] truncate">
                            {post.articleTitle ?? post.articleId.slice(0, 8)}
                          </span>
                        </TableCell>
                        <TableCell className="text-muted-foreground">
                          {post.siteName ?? post.siteId.slice(0, 8)}
                        </TableCell>
                        <TableCell>
                          ...
                          {post.errorMessage && post.status === 'failed' && (
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <p className="mt-1 max-w-[200px] cursor-default truncate text-xs text-destructive">
                                  {post.errorMessage}
                                </p>
                              </TooltipTrigger>
